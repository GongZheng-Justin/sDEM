...
!********************************************************************************
  subroutine  DEMS_Initialize(this,chDEMPrm)
    ...
    write(ch,"(A)") 'mkdir -p '//DEM_opt%Res_Dir//' '//DEM_opt%RestartDir//' 2> /dev/null'
    ...
    if(nrank==0) then
      ...
      if( DEM_opt%CF_Type == CFT_LSD ) then
        write(ch,"(A)") "linear spring-dashpot with limited tangential displacement"
      elseif( DEM_opt%CF_Type == CFT_nLin ) then
        write(ch,"(A)") "non-linear visco-elastic model with limited tangential displacement"
      endif
      call DEMLogInfo%OutInfo("Contact force model is "//trim(ch), 2 )

      if(DEM_opt%PI_Method==PIM_FE) then
        write(ch,"(A)") "Forward Euler             "
      elseif(DEM_opt%PI_Method==PIM_AB2) then
        write(ch,"(A)") "Adams Bashforth: 2nd Order"
      elseif(DEM_opt%PI_Method==PIM_AB3) then
        write(ch,"(A)") "Adams Bashforth: 3nd Order"
      endif
  ...
  subroutine Write_DEM_Opt_to_Log()
    ...
    write(Res_Dir,"(A)") DEM_opt%Res_Dir 
