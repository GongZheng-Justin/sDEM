...
!********************************************************************************
  subroutine  DEMS_Initialize(this,chDEMPrm)
    ...
    call DEMLogInfo%InitLog(d_DEMLogInfo_unit, DEM_opt%Res_Dir, DEM_opt%RunName, &
                            DEM_opt%LF_file_lvl, DEM_opt%LF_cmdw_lvl)
  ...
  subroutine DEMS_preIteration(this)
    ...
    if(DEMContactSearchPW%UpdateNearPrtclsPW(this%iterNumber)) then
      if(nrank==0 .and. mod(this%iterNumber,DEM_opt%Wall_max_update_iter)/=0 ) then
  ...
  subroutine Write_DEM_Opt_to_Log()
    ...
    write(d_DEMLogInfo_unit, nml=DEMOptions)
