module Prtcl_Comm
  ...
  real(RK),dimension(6),save::dx_pbc
  real(RK),dimension(6),save::dy_pbc
  real(RK),dimension(6),save::dz_pbc  

  real(RK),save:: xst0_cs,xst1_cs,xst2_cs
  real(RK),save:: xed0_cs,xed1_cs,xed2_cs
  real(RK),save:: yst0_cs,yst1_cs,yst2_cs
  real(RK),save:: yed0_cs,yed1_cs,yed2_cs
  real(RK),save:: zst0_cs,zst1_cs,zst2_cs
  real(RK),save:: zed0_cs,zed1_cs,zed2_cs
  ...
  integer,dimension(:),allocatable,public:: GhostPFix_id
  integer,dimension(:),allocatable,public:: GhostPFix_pType
  type(real4),dimension(:),allocatable,public:: GhostPFix_PosR
...
contains
  ...
  subroutine PC_InitComm(this)
    ...
    ! locals
    integer::numPrtcl,mCS,msend,mFixedCS
    integer::iErr01,iErr02,iErr03,iErr04,iErr05,iErr06,iErrSum
    real(RK)::maxD,cell_len,vol_tot,vol_ghost_cs,vol_sendlist
    ...
    allocate(GhostPFix_id(mFixedCS),    Stat=iErr01)
    allocate(GhostPFix_pType(mFixedCS), Stat=iErr02)
    allocate(GhostPFix_PosR(mFixedCS),  Stat=iErr03)
    allocate(sendlist(msend),   Stat=iErr04)
    iErrSum=abs(iErr01)+abs(iErr02)+abs(iErr03)+abs(iErr04)
    if(iErrSum/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"PC_InitComm","Allocation failed1")
    ...
    this%msend=msend
    if(allocated(sendlist)) deallocate(sendlist);
    allocate(GhostP_id(mCs),    Stat=iErr01)
    allocate(GhostP_pType(mCs), Stat=iErr02)
    allocate(GhostP_PosR(mCs),  Stat=iErr03)
    allocate(GhostP_linVel(mCs),Stat=iErr04)
    allocate(GhostP_rotVel(mCS),Stat=iErr05)
    allocate(sendlist(msend),   Stat=iErr06)
    iErrSum=abs(iErr01)+abs(iErr02)+abs(iErr03)+abs(iErr04)+abs(iErr05)+abs(iErr06)
    if(iErrSum/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"PC_InitComm","Allocation failed2")
  ...
  subroutine PC_Comm_For_Cntct(this)
    ...
    ! locals
    integer::i,ierror,request(4)
    integer::nsend,nsend2,nrecv,nrecv2,nsendg,ng,ngp
    ...
    CASE(x_axis)     ! ccccccccccccccccccccccccc  x-axis  ccccccccccccccccccccccccccc
      ...
      IF(DEM_decomp%ProcNgh(3)==nrank) THEN  ! neighbour is nrank itself.
        do i=1,ngp    ! consider the previous ghost particle firstly
          py=GhostP_PosR(i)%y
          if(py>=yed0_cs ) then
            ng=ng+1
            if(ng>GPrtcl_list%mGhost_CS) call this%reallocate_ghost_for_Cntct(ng)
      ...
      IF(DEM_decomp%ProcNgh(4)==nrank) THEN  ! neighbour is nrank itself.
        do i=1,ngp    ! consider the previous ghost particle firstly
          py=GhostP_PosR(i)%y
          if(py<=yst2_cs .and. py>=yst1_cs) then
            ng=ng+1
            if(ng>GPrtcl_list%mGhost_CS) call this%reallocate_ghost_for_Cntct(ng)
            GhostP_id(ng)     = GhostP_id(i)
            GhostP_pType(ng)  = GhostP_pType(i)
  ...
  subroutine PC_Comm_For_Exchange(this)
    ...
    ! locals
    integer::i,id,ierror,request(4),nlocal,nlocalp
      ...
      ELSE
        do i=1,nlocal
          ...
          if(py >= yed1_cs) then
            ...
            nsend(2)= nsend(2) +nlink*4+ this%Prtcl_Exchange_size+1 ! The final "1" stands for END_OF_PRTCL
          endif
        enddo
      ENDIF
  ...
  subroutine PC_Comm_For_Cntct_fixed(this)
    ...
    ! locals
    integer::i,ierror,request(4),GhostFixedCS_size
    integer::nsend,nsend2,nrecv,nrecv2,nsendg,ng,ngp
    real(RK),dimension(:),allocatable::buf_send,buf_recv
    integer,dimension(MPI_STATUS_SIZE) :: SRstatus
    real(RK)::px,py,pz
