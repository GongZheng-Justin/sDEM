module Prtcl_Hrchl_Munjiza
  ...
  integer,save::nlocal,nlocalp,nghost
...
contains
  ...
  subroutine Init_Munjiza_Hrchl(this )
    ...
    minD   = two*minval( DEMProperty%Prtcl_PureProp%Radius )
    ...
    DO idh = 1, numLevels
      numPrtcl_lvl = 0
      minD_lvl = maxD_lvl/two; if(idh==numLevels) minD_lvl=zero

      do i = 1, DEM_opt%numPrtcl_Type
        Diam = two * DEMProperty%Prtcl_PureProp(i)%Radius
    ...
    DO idh = 1, numLevels
      ...
      minD_lvl = maxD_lvl/two 
      if( idh == numLevels) minD_lvl = zero
