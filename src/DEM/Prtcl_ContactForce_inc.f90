    ...
    vrn  = Vrij .dot. Norm_v
    if(VelRel_Init(ind)<-one)VelRel_Init(ind)=abs(vrn)
    ...
    case(CFT_LSD)
      Vel_in=max(VelRel_Init(ind),0.01_RK)
      k_n= -Prop_ij%StiffnessCoe_n*Vel_in**0.4_RK
      d_n= -Prop_ij%DampingCoe_n*Vel_in**0.2_RK
      fn =  k_n*ovrlp +d_n* vrn
      k_t= -Prop_ij%StiffnessCoe_t*Vel_in**0.4_RK
      d_t=  zero  ! No equation is considered for tangential damping yet
