...
contains
  ...
  subroutine G_InitAllocate(this)
    ...
    if(nrank/=0) return
    chFile = trim(DEM_opt%Res_Dir)//"WallsFor"//trim(DEM_opt%RunName)//".backup"
    nUnitFile = d_backup_geom_unit
    open(unit=nUnitFile, file=chfile,status='replace',form='formatted',IOSTAT=myistat)
    if(myistat/=0.and.nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"G_InitAllocate","Cannot open file: "//trim(chFile))
  ...
  subroutine G_add_PlaneWall(this, p1, p2, p3, p4 , user_id ,prop_type,  both, infinite, t_vel_t)
    ...
    type(real3)::t_vel, ln
    integer:: nUnitFile,myistat,i
    character(64) :: chFile        
 
    IF(this%nPW_local == MaxWallSize) THEN
      MaxWallSize  = int( real(MaxWallSize, RK) *1.2_RK) +1
      call move_alloc(this%pWall, wall_temp)
      allocate(this%pWall(MaxWallSize),Stat=myistat) 
      if(myistat/=0) call DEMLogInfo%CheckForError(ErrT_Abort,"G_add_PlaneWall","Reallocation failed, 2 ")
    ...
    IF(wall%IsInDomain(pmin, pmax)) THEN
      ...
      if(myistat/=0 .and. nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"G_add_PlaneWall","Cannot open file: "//trim(chFile))
      write(nUnitFile,* ) p1
      write(nUnitFile,* ) p2
      write(nUnitFile,* ) p3
      write(nUnitFile,* ) p4
  ...
  subroutine G_MakeGeometry(this,chFile)
    ...
    !locals
    integer:: i,nplane,nUnitFile,myistat
  ...
  !sandbox
    ...
    elseif(DEM_Opt%GeometrySource ==1) then  ! add the geometry from the NAMELIST "&GeometryMakingParam"
      ...
      if(myistat /= 0 .and. nrank==0) call DEMLogInfo%CheckForError(ErrT_Abort,"G_MakeGeometry","Cannot open file:"//trim(chFile))
      read(nUnitFile, nml=GeometryMakingNumPlane)
      if(nplane<1) then
        close(nUnitFile, IOSTAT=myistat);return
      endif
