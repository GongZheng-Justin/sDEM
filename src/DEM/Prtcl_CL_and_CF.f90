...
contains
  ...
  subroutine CL_RemvReleased(this)
    ...
    do pid=1,GPrtcl_list%nlocal ! Modify this part by Zheng Gong, at 202-09-09
      prev=0
      n=Bucket(pid)
      do while(n>0)
        NextI=Next(n)
  ...
  subroutine CL_Add_RestartCntctlink(this,pid1,ncv,CntctVec,TanDel_Un)
    ...
    DO j=1,ncv
      ...
      IF(pid2>0) THEN
        ...
        if(gid1>gid2 .and. dist<=zero) cycle
      ENDIF
      ...
      if(pid2>0 .and. gid1<gid2 .and. dist<=zero) then  ! gid2 is also within this processor
        id_i(NextI)= gid1
  ...
  subroutine CL_printCL(this,itime)
    ...
    if(nrank==0) then  
      open(nUnit,file=filename,status='replace',form='formatted')
      close(nUnit)
    endif
    call MPI_BARriER(MPI_COMM_WORLD,ierror)
    do i=0,nproc-1
      if(nrank==i) then
        open(nUnit,file=filename,status='old',position='append',form='formatted')
        DO pid=1,GPrtcl_list%nlocal
          n = Bucket(pid)
          do while(n>0)
            write(nUnit,*)nrank,GPrtcl_id(pid),id_j(n),TanDelta(n),VelRel_Init(n)
            n = Next(n)
          enddo
     
          n = Head_cp(pid)
          do while(n>0)
            write(nUnit,*)nrank,GPrtcl_id(pid),id_i(n),TanDelta(n),VelRel_Init(n)
            n = Next_cp(n)
          enddo
        ENDDO
        close(nUnit)
  ...
  subroutine ContactForce_PP(pid,pjd,ind,ovrlp)
    ...
    real(RK)::ri,rj,fn,ft,vrn,ft_fric,k_n,d_n,k_t,d_t,normTan1,normTan2,w_hat_mag,Vel_in
