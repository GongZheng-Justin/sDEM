module Prtcl_LogInfo
...
#endif
  use Prtcl_TypeDef,only:GetFileUnit
  implicit none
  private
  ...
  type(LogInfo),public :: DEMLogInfo
...
contains
  ...
  subroutine LI_OpenFile(this,chFile,RunName)
    ...
    this%nUnit= GetFileUnit()
    write(this%chLogFileName,"(A)") chFile
    if(nrank==0) then
      open(file = chFile, unit = this%nUnit, status='replace', IOSTAT=ierror)
    ...
    if(nrank/=0) open(file=chFile, unit=this%nUnit, status='old', IOSTAT=ierror)
  end subroutine 
  ...
  subroutine LI_CloseFile(this)
    implicit none
    class(LogInfo) this
    close(this%nUnit,IOSTAT=myistat)
  ...
  subroutine LI_OutInfo( this, chInfo, lvl , no_bull )
    ...
    if( lvl <= this%rprt_lvl_file )then
      if(lvl == 1 )write(this%nUnit, *,IOSTAT=myistat )
      if(l_no)then
        write(this%nUnit, ch101,IOSTAT=myistat ) trim(chInfo)    
      else
        write(this%nUnit, ch100,IOSTAT=myistat ) bullet(lvl), trim(chInfo)    
      endif
    endif
