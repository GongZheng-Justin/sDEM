...
contains
  ...
  subroutine InitContactSearchPW(this )
    ...
    type(real3):: wallvel
    integer:: i,nw,iErr1,iErr2,iErr3,iErrSum
    integer:: max_numCntcts,ierror
    ...
    if(iErrSum/=0 ) call DEMLogInfo%CheckForError(ErrT_Abort,"InitContactSearchPW","Allocation failed ")
    ...
    this%DeltaXmax1 =  maxval( DEMProperty%Prtcl_PureProp%Radius) * DEM_opt%Wall_neighbor_ratio
    this%DeltaXmax2 =  maxval( DEMProperty%Prtcl_PureProp%Radius) *(DEM_opt%Wall_neighbor_ratio+one)
  ...
  subroutine Reallocate_Bucket(this,nB_new)
    ...
    Bucket_PWCS(sizep+1:sizen)=0     ! Added at 11:24, 2020-09-11, Gong Zheng
    deallocate(IntVec)
  ...
  !**********************************************************************
  logical function UpdateNearPrtclsPW(this,iterNumber)
    implicit none
    class(ContactSearchPW) :: this
    integer,intent(in):: iterNumber

    ! locals
    integer:: nw,i,numIter,wid, f_ind, l_ind, n, old_n,nNear,nextI,ierror
    type(real3):: min_point, max_point, pmin_point, pmax_point
    integer,allocatable,dimension(:) :: temp
    real(RK)::diffx,diffy,diffz,max_v,max_a,t,maxreal 

    ! checks if in this iteration the neighbour list should be updated
    if(iterNumber<this%Next_iter_update) then
      UpdateNearPrtclsPW = .false.
      return
    end if
    call this%Init_PWCS_List()
    ...
    do wid = 1,nw
      ...
      pmin_point = min_point - this%DeltaXmax2*one_r3
      pmax_point = max_point + this%DeltaXmax2*one_r3
    ...
    if(nrank==0) then
      t = (sqrt(max_v**2+ two * max_a*this%DeltaXmax1)-max_v)/max_a
    ...
    UpdateNearPrtclsPW = .true.
    
  end function UpdateNearPrtclsPW
