...
contains
  ...
  subroutine Initilize(this,chFile)
    ...
    ch= 'mkdir -p '//trim(adjustl(DumpPrtclDir))//' 2> /dev/null'
    if(nrank==0) then 
      call system(trim(adjustl(ch)))
      write(d_DEMLogInfo_unit, nml=DumpPrtclOptions)
    endif
  ...
  subroutine PrtclVarDump(this,itime)
    ...
    open(unit=75,file=trim(chFile),status='replace',form='unformatted',access='stream',IOSTAT=myistat)
    IF(myistat/=0) THEN
      call DEMLogInfo%CheckForError(ErrT_Abort,"PrtclVarDump","Cannot open file: "//trim(chFile))
    ELSE
      write(75)DumpPrtclVec(1:nDumpPrtclSize)
    ENDIF
    close(75,IOSTAT=myistat)
